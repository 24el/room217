...
class Comment extends Model{
    ...
    public function calcRating($userId, $flag){
        $user = User::find()->where(['id' => $userId])->one();
        $marks = Comments::find()->where(['user_id' => $userId])->all();
        $rating = 0;
        $feedbNum = 0;
        foreach($marks as $mark){
            $isUserExec = Orders_users::find()->where(['user_id' => $mark->user_id])->andWhere(['order_id' => $mark->order_id])->exists();

            if($flag == 'customer' && !$isUserExec){
                $rating += $mark->rating;
                $feedbNum++;
            }elseif($flag == 'employee' && $isUserExec){
                $rating += $mark->rating;
                $feedbNum++;
            }
        }
        if($flag == 'customer' && $rating){
            $user->customer_rating = $rating/$feedbNum;
            $user->save();
        }
        elseif($flag == 'emoloyee' && $rating){
    ...
    public static function hasUserComment($orderId, $userId = null){
        $UsCom = Comments::find()->where(['order_id' => $orderId])
            ->andWhere(['creator_id' => isset($userId) ? $userId : \Yii::$app->user->identity->id])->exists();
        return $UsCom;
    }
}